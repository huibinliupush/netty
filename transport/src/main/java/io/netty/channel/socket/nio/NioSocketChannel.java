...
public class NioSocketChannel extends AbstractNioByteChannel implements io.netty.channel.socket.SocketChannel {
    ...
    /**
     * 关闭连接的「读」这个方向，如果此时Socket接收缓冲区有已接收的数据，则会将这些数据统统丢弃。
     * 如果后续再收到新的数据，虽然也会对这些数据进行ACK确认，但是会悄悄丢弃掉。所以在这种情况下，
     * 对端虽然收到了ACK确认，但是这些以发送的数据可能已经被悄悄丢弃了。
     * 关闭读通道的方法在JDK NIO中对应于SocketChannel#shutdownInput()，这里需要注意的是此方法并不会发送FIN。
     *
     * */
    @Override
    ...
    protected void doFinishConnect() throws Exception {
        // 确认 channel 是否真正的 connected,否则 throw error
    ...
    private void adjustMaxBytesPerGatheringWrite(int attempted, int written, int oldMaxBytesPerGatheringWrite) {
        ...
        if (attempted == written) {
            ...
            // 如果连 4K 都写不进去了，那么就缩小 MaxBytesPerGatheringWrite
            ((NioSocketChannelConfig) config).setMaxBytesPerGatheringWrite(attempted >>> 1);
        }
    }
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        // 注意这里每次是允许直接一次性批量发送最多 1024 个 ByteBuf
        ...
        do {
            ...
            switch (nioBufferCnt) {
                // ChannelOutboundBuffer 中的 ByteBuf 和 fileRegion 这里是分开处理的
                case 0:
    ...
    private final class NioSocketChannelConfig extends DefaultSocketChannelConfig {
        ...
        int getMaxBytesPerGatheringWrite() {
            // 创建 NioSocketChannel 的时候计算，SO_SNDBUF 设置的发送缓冲区大小 * 2 作为 最大写入字节数
